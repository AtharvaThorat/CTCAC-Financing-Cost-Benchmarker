...
4. **Validation:**
* Verify that the line items sum up to the Total listed in the sheet.
...
* **Grid Scanner (for Units):** The script scans the "Application" tab row-by-row looking for keywords like "Total Units" or "Unit Count" to find the exact project size.
...
* **Hard Costs:** It prioritizes "New Construction." If that is empty (0), it automatically falls back to "Rehabilitation" costs and flags this action.
...
4. It saves the final data to **`summary_output.csv`**.

---

## 5. Explanation in Basic Terms (EL5)
...
3. **The Detective (Finding Data):**
* It looks for the words "Total Units." When it finds them, it grabs the number next to it.
